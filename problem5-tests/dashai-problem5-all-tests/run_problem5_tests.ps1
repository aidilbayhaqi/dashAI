...
Run-Step "1. Compile backend dan migration" {
    docker compose exec api python -m compileall src migrations
}
...
$FrontendMount = "type=bind,source=$FrontendPath,target=/frontend,readonly"
...
if (-not $SkipBuild) {
    ...
    try {
        docker compose run --rm --no-deps --entrypoint sh frontend `
            -lc 'find /app/.next -mindepth 1 -maxdepth 1 -exec rm -rf {} +'
        if ($LASTEXITCODE -ne 0) {
            throw "Gagal membersihkan .next."
        }
